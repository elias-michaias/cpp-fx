...
private:
  ...
  static void destroy_fn(void *p) noexcept { static_cast<Tb *>(p)->~Tb(); }

  template <typename Tb>
  static void move_fn(void *dst, void *src) noexcept {
...
template <typename... Es> struct PromiseBase : PromiseAbortBase {
  ...
  static void *operator new(std::size_t n) {
    ...
    *reinterpret_cast<std::pmr::memory_resource **>(
        static_cast<unsigned char *>(raw) + n) = mr;
    return raw;
  }
...
struct MonotonicResource : std::pmr::memory_resource {
...
private:
  ...
  auto run_push_composite([[maybe_unused]] H &h, void * ,
