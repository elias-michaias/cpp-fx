...
auto safe_div_or_zero(int a, int b) -> Log::Fx<int> {
...
auto silent_div(int a, int b) -> Fx<int> {
...
int main() {
  auto log_sink = handler<Log>([](Log, auto r) { r({}); });
  auto ask_const = [](std::string val) {
    return handler<Ask>([val](Ask, auto r) { r(val); });
  };

  // 1a. ask_silently: Log absorbed, caller only provides Ask.
  auto r1a = ask_silently("city", "City: ").run(ask_const("London"));
  assert(r1a == "London");
  ...
  auto log_cap = handler<Log>([&](Log e, auto r) {
    logged.push_back(e.message);
    r({});
  });
  auto r1b_ok = safe_div_or_zero(10, 2).run(log_cap);
  auto r1b_bad = safe_div_or_zero(10, 0).run(log_cap);
  assert(r1b_ok == 5);
  assert(r1b_bad == 0);
  ...
  auto fully_stripped = handle<Ask>(
      handle<Log>(ask_and_log("x", "x: "),
                  handler<Log>([](Log, auto r) { r({}); })),
      handler<Ask>([](Ask, auto r) { r(std::string{"stripped"}); }));
  auto r4 = fully_stripped.run(); // no handlers needed
  assert(r4 == "stripped");
