...
let logged_div(int a, int b) -> Row<Log, Fail>::Fx<int> {
  perform(Log{.message = "computing " + std::to_string(a) + "/" + std::to_string(b)});
  let q = co_await safe_div(a, b);
  perform(Log{.message = "result: " + std::to_string(q)});
  co_return q;
}

// Level 3: calls ask_number (Ask propagates) and logged_div (Log+Fail propagate).
// Own effect list = Ask ∪ Log ∪ Fail = All.  The caller supplies all three handlers.
let compute_ratio() -> All::Fx<std::string> {
  ...
  co_return std::to_string(num) + "/" + std::to_string(den) + " = " + std::to_string(q);
}
...
int main() {
  ...
  let r1 = compute_ratio().run(scripted_ask({"12", "4"}), silent_log, fail_neg1);
  assert(r1 == "12/4 = 3");
  ...
  let r2 = compute_ratio().run(
      scripted_ask({"6", "0"}),
      silent_log,
      handler<Fail>([&](Fail e, auto r) { caught_reason = e.reason; r(-1); }));
  ...
  compute_ratio().run(
      handler<Ask>([&](Ask e, auto r) {
        prompts.push_back(e.prompt);
        r("1");  // answer with "1" so stoi succeeds
      }),
      handler<Log>([&](Log e, auto r) { log_entries.push_back(e.message); r({}); }),
      fail_neg1);
  ...
  let r4a = compute_ratio().run(scripted_ask({"10", "2"}), silent_log, fail_neg1);
  let r4b = compute_ratio().run(scripted_ask({"7",  "7"}), silent_log, fail_neg1);
  assert(r4a == "10/2 = 5");
  assert(r4b == "7/7 = 1");
  ...
  compute_ratio().run(
      handler<Ask>([&](Ask, auto r) { ++ask_fires; r("0"); }),
      handler<Log>([&](Log, auto r) { ++log_fires; r({}); }),
      handler<Fail>([&](Fail, auto r) { ++fail_fires; r(-1); }));
  ...
  std::cout << "5. effect counts: Ask=" << ask_fires
            << " Log=" << log_fires << " Fail=" << fail_fires << "\n";
